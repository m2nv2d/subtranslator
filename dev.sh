...
if [ -n "$LOG_CONFIG" ]; then
    uv run uvicorn --app-dir ./src --reload --host 0.0.0.0 --port $PORT --log-config $LOG_CONFIG main:app
else
    uv run uvicorn --app-dir ./src --reload --host 0.0.0.0 --port $PORT main:app
fi